...
public class ObjectSpawner1 : MonoBehaviour
{
    ...
    void SpawnObjects()
    {
        TMP_InputField longiNumLeft = GameObject.Find("Longi_Number_L").GetComponent<TMP_InputField>(); // ok
        TMP_InputField longiNumRIght = GameObject.Find("Longi_Number_R").GetComponent<TMP_InputField>(); // ok
        TMP_Dropdown longiTypeLeft = GameObject.Find("Longi_Type_L").GetComponent<TMP_Dropdown>(); // ok
        TMP_Dropdown longiTypeRight = GameObject.Find("Longi_Type_R").GetComponent<TMP_Dropdown>(); // ok
        TMP_InputField longiDis = GameObject.Find("Longi_Distance").GetComponent<TMP_InputField>(); // ok
        TMP_Dropdown slotHoleTypeLeft = GameObject.Find("Slot_Hole_Type_L").GetComponent<TMP_Dropdown>(); // ok
        TMP_Dropdown slotHoleTypeRight = GameObject.Find("Slot_Hole_Type_R").GetComponent<TMP_Dropdown>(); // ok
        TMP_Dropdown rHoleLeft = GameObject.Find("R_Hole_L").GetComponent<TMP_Dropdown>(); // ok
        TMP_Dropdown rHoleRight = GameObject.Find("R_Hole_L").GetComponent<TMP_Dropdown>(); // ok
        TMP_Dropdown plateTypeLeft = GameObject.Find("Plate_Type_L").GetComponent<TMP_Dropdown>(); // ok
        TMP_Dropdown plateTypeRight = GameObject.Find("Plate_Type_R").GetComponent<TMP_Dropdown>(); // ok
        TMP_Dropdown plateLocation = GameObject.Find("Plate_Location").GetComponent<TMP_Dropdown>(); // ok
        ...
        float randomDis = ConvertInputToFloat(longiDis) * 0.05F; // 교체 완료
        ...
        //int Longi1 = Random.Range(1, 26);
        //int Longi2 = Longi1;
        int Longi1 = ConvertInputToInt(longiNumLeft); // 교체 완료
        int Longi2 = ConvertInputToInt(longiNumRIght); // 교체 완료
        ...
        string randomLongiSuffix1 = GetSelectedDropdownValue(longiTypeLeft); // 교체 완료
        string randomLongiSuffix2 = GetSelectedDropdownValue(longiTypeRight); // 교체 완료
        ...
        string randomSlotHoleSuffix1 = GetSelectedDropdownValue(slotHoleTypeLeft); // 교체 완료
        string randomSlotHoleSuffix2 = GetSelectedDropdownValue(slotHoleTypeRight); // 교체 완료
        ...
        string randomPlateSuffix1 = GetSelectedDropdownValue(plateTypeLeft); // 교체 완료
        string randomPlateSuffix2 = GetSelectedDropdownValue(plateTypeRight); // 교체 완료
        ...
        int plateChoice = GetDropdownValueAsInt(plateLocation); // 교체 완료
        ...
        // make R hole
        int locationValueLeft = GetDropdownValueAsInt(rHoleLeft); // 교체 완료
        int locationValueRight = GetDropdownValueAsInt(rHoleRight); // 교체 완료
        makeRHole(Longi1, randomSlotHoleSuffix1, randomDis, locationValueLeft);
        makeRHole(Longi2, randomSlotHoleSuffix2, randomDis, locationValueRight);
    ...
    void makeRHole(int index, string sloteHoleSuffix, float dis, int locationValue)
    {
        // if rand == 0 , not make
    ...
    private int ConvertInputToInt(TMP_InputField inputField)
    {
        ...
        if (int.TryParse(inputField.text, out result))
        {
            ...
            {
                Debug.LogWarning("Input is not within the range 1-26: " + result);
                return 0; // 범위 밖일 때 반환할 기본값 (필요에 따라 조정 가능)
            }
        }
        else
        {
            Debug.LogWarning("Input is not a valid integer: " + inputField.text);
            return 0; // 변환 실패 시 반환할 기본값 (필요에 따라 조정 가능)
        }
    }
    ...
    private float ConvertInputToFloat(TMP_InputField inputField)
    {
        ...
        if (float.TryParse(inputField.text, out result))
        {
            ...
            {
                Debug.LogWarning("Input is not within the range 500-850: " + result);
        ...
        else
        {
            ...
            return 0f; // 변환 실패 시 반환할 기본값 (필요에 따라 조정 가능)
        }
    }
    ...
    private int GetDropdownValueAsInt(TMP_Dropdown dropdown)
    {
        ...
        switch (selectedText)
        {
            ...
            case "Right":
                return 2;
            default:
                Debug.LogWarning("Unexpected dropdown value: " + selectedText);
